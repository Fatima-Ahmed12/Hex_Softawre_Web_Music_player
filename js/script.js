...
mainAudio.addEventListener("timeupdate",(e)=>{
    ...
    repeatBtn.addEventListener("click",()=>{
        ...
        let getText = repeatBtn.innerText; // geting innerText of icon
        //lets do different changes on different icon click using switch
         switch(getText){
           ...
           break;
           case "repeat_one"://if icon is repeat_one then change to shuffle
        ...
        mainAudio.addEventListener("ended",()=>{
            ...
            //lets do different changes on different icon click using switch
             ...
             switch(getText){
            case "repeat": //if this icon is repeat simply we call the nextMusic function so the next song will play
            nextMusic();
           break;
           case "repeat_one"://if icon is repeat_one then change to repeat_one then we'll the current playing song and current time to 0 so song will play from begining 
         ...
         do{
           ...
           while(musicIndex ==randIndex);  //this loop run untilthe next random number wont be the same current music index
    ...
    showMoreBtn.click();
     ...
     for(let i=0; i< allMusic.length; i++){
        //let's pass the sonng name ,artist from the array to li
        let liTag = ` <li li-index="${i+1}">
                      ...
                      <span id="${allMusic[i].src}" class="audio-duration">2:18</span>
                    </li>`;
       ...
       liAudioTag.addEventListener("loadeddata",()=>{
        ...
        //adding t duration attribute which we'll use bellow
     ...
     // lets work on play particular song on click
        ...
        function playingNow(){
         for(let j = 0; j < allLiTags.length; j++){
            ...
            //lets remove playing class from all other li expect the last one which
            if(allLiTags[j].classList.contains("playing")){
                allLiTags[j].classList.remove("playing");
              //let's get that audio duration value and pass to .audio-duration  innerText
                let adDuration= audioTag.getAttribute("t-duration");
                audioTag.innerText = adDuration; //passing t duration value to the audio duration innerText
            }
